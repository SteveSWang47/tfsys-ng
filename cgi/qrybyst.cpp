...
int main()
{
    ...
    if(std::regex_match(str, std::regex(R"([0-9]{5})")))
    {
        ...
        mysqlpp::Query qry = conn.query("select * from recs where setid="+str+" order by time desc offset " + std::to_string(pag*50) + " rows fetch first 50 rows only;");
